...
3. Write `BrandTokens.ts` with the exact typography, color, motion, and spacing tokens.
4. Write enough `film.ts` content for the thumbnail: project name, chosen variant, a representative script line, the cover-frame intent, and the vision-close tagline.
...
If the user says "warmer," "too SaaS," "more premium," "less dark," or anything similar, treat that as valid design feedback. Update `design.md` and `BrandTokens.ts`, re-render `design-thumbnail.png`, and repeat until approved. Only then proceed to Phase 5.
