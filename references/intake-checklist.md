...
**Example (generic dev tool):**

> Reflex is a debugging environment for frontend engineers that captures the full state of a bug — DOM, network, console, user actions — in a single shareable link.
>
> Most bugs are reproduction problems, not logic problems; Reflex makes "works on my machine" obsolete by giving engineers a time-travel debugger that another human can replay.
