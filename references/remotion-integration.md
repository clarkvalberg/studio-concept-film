...
- A shared `MotionFloor` layer that adds subtle motion to otherwise held scenes so the hook does not become a slide
...
The template includes a subtle `MotionFloor` overlay for held or placeholder scenes. Treat it as a floor, not the motion plan. The approved motion board still needs real visible state change through scene props or custom scene components.
