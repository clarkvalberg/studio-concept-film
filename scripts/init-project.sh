...
if [ $# -lt 1 ]; then
  ...
  echo "Example: $0 ~/concepts/signatures-law"
  exit 1
fi
...
export const tokens = {
  typography: {
    display: { family: 'Inter', weight: 700, fallback: 'system-ui, sans-serif' },
...
TOKENS
  ...
  cat > "$REMOTION_DIR/src/compositions/DesignThumbnail.tsx" << 'DESIGN'
import React from 'react';
...
echo ""
echo "Next steps:"
echo "  1. Skill writes design tokens to $PROJECT_DIR/remotion/src/compositions/shared/BrandTokens.ts"
echo "  2. Skill renders $PROJECT_DIR/out/design-thumbnail.png for Phase 4 review"
echo "  3. Skill writes the film data to $PROJECT_DIR/remotion/src/data/film.ts"
echo "  4. Skill places voiceover audio at $PROJECT_DIR/remotion/public/audio/hook.mp3"
echo "  5. Skill places product screens at $PROJECT_DIR/remotion/public/screens/"
